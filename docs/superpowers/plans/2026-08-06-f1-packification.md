...
S1-S5 закрыты. ⚠ Обнаружено в Ф4: **S4 работал наполовину** — alias `@pack` не резолвил
подпути ни в одном бандлере, поэтому «второй жилец» на S5 доказывал контракт только
контентом. Починено в Ф4 (`packs/_active`, `aa91223`). Хвосты Ф1 живы: спайн `modules.ts`
и `lib/intake/questions.ts` в ядре, packs не подняты на корень репо.
