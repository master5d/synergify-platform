...
After module 02 you can set up the **advanced status line** (see 02-setup-guide.md → "Advanced setup"):
...
## 🔭 Where to look at real implementations

When you need a working example rather than advice: two open catalogues of LLM apps, from simple bots to multi-agent systems. Both are in English, both alive (weekly commits), and the code is there to read and borrow.

```
awesome-llm-apps      github.com/Shubhamsaboo/awesome-llm-apps
                      Apache-2.0 · RAG, AI agents, multi-agent teams, MCP, voice agents

awesome-ai-apps       github.com/Arindam200/awesome-ai-apps
                      MIT · hands-on examples, guides and recipes: from a chatbot to an agent
```

Look for structure, not snippets: where the agent's boundaries are, what got pushed into tools, how the result is measured.
