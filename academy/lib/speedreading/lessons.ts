...
1. Откройте \`/trenazhery/test/\` и пройдите замер один раз в обычном темпе. Запишите две цифры: wpm и процент понимания. Не переделывайте, если результат не понравился — это и есть точка отсчёта.
...
1. Open \`/trenazhery/test/\` and take one measurement at your normal pace. Write down two numbers: wpm and comprehension percentage. Do not retake it because you dislike the result — that result is the starting point.
...
2. Откройте \`/trenazhery/rsvp/\`. Поставьте скорость примерно на 10% выше вашей базовой из первого урока. Прочитайте 3 минуты. Не гонитесь за максимальной цифрой: цель — не рекорд, а опыт чтения без возможности вернуться.
...
2. Open \`/trenazhery/rsvp/\`. Set the rate about 10% above the baseline from lesson one. Read for 3 minutes. Do not chase the highest number — the goal is the experience of reading with no way back, not a record.
...
2. Откройте \`/trenazhery/rsvp/\`, поставьте 280-320 wpm — заведомо выше речевого потолка. Читайте 3 минуты, затем своими словами перескажите вслух главную мысль в двух предложениях. Если пересказ не получился — снизьте скорость на 40 wpm и повторите.
...
2. Open \`/trenazhery/rsvp/\` and set 280-320 wpm, deliberately above the speech ceiling. Read for 3 minutes, then say the main idea out loud in two sentences in your own words. If you cannot, drop 40 wpm and repeat.
...
1. \`/trenazhery/schulte/\` — 4 подхода по таблице 5×5. Держите взгляд в центре, ищите числа периферией. Записывайте время каждого подхода; между подходами 30 секунд паузы.
...
1. \`/trenazhery/schulte/\` — 4 rounds on a 5×5 table. Keep your gaze centred and find the numbers peripherally. Record each round's time; rest 30 seconds between rounds.
...
5. Раз в неделю проверяйте себя на \`/trenazhery/test/\` — там понимание считается вместе со скоростью, и это единственный способ заметить, что вы разогнались за счёт смысла.
...
5. Once a week, check yourself on \`/trenazhery/test/\`, where comprehension is scored alongside rate. It is the only way to notice that you sped up by spending meaning.
